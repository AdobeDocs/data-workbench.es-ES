...
Realización de cambios en la variable [!DNL Log Processing Mode.cfg] , incluida la adición o eliminación de orígenes, no causa el reprocesamiento de los datos.
...
1. Mientras trabaja en el perfil del conjunto de datos, abra el [!DNL Profile Manager] y haga clic en **[!UICONTROL Dataset]** para mostrar su contenido.
   ...
   >[!NOTE]
   >
   >Si la variable [!DNL Log Processing Mode.cfg] no se encuentra en el directorio del perfil deseado, debe copiar este archivo desde el directorio base del equipo del servidor de Data Workbench en el directorio del perfil.
...
1. Haga clic con el botón derecho en la marca de verificación situada junto al nombre del archivo de configuración y haga clic en **[!UICONTROL Make Local]**. Aparece una marca de verificación para este archivo en la variable [!DNL User] para abrir el Navegador.
...
1. Edite los parámetros en el archivo de configuración utilizando la siguiente tabla como guía.
   ...
   >[!NOTE]
   >
   >Algunos de los parámetros de la variable [!DNL Log Processing Mode.cfg] tiene nombres que incluyen [!DNL Fast Input] o [!DNL Fast Merge]. [!DNL Fast Input]hace referencia a la fase de procesamiento de registros de la construcción del conjunto de datos y es responsable de aproximadamente la mitad del tiempo total de procesamiento del conjunto de datos. [!DNL Fast Merge] hace referencia a la fase de transformación de la construcción del conjunto de datos solo cuando va precedida del procesamiento de registros. [!DNL Fast Merge] no se produce durante la retransformación que resulta de modificar un [!DNL Transformation Dataset Configuration] archivo. Like [!DNL Fast Input], [!DNL Fast Merge] también es responsable de aproximadamente la mitad del tiempo de procesamiento del conjunto de datos.
   ...
   <tr> 
      ...
      <td colname="col2"> <p>Un parámetro de ajuste que especifica la proporción de bytes de registro totales a no leídos en la que entra el sistema <span class="wintitle"> Entrada rápida</span> modo (y <span class="wintitle"> Fusión rápida</span>) en lugar de procesar los datos en tiempo real. </p> <p> El valor predeterminado es 200, lo que significa que el sistema entra en <span class="wintitle"> Entrada rápida</span> del modo de tiempo real cuando los datos de registro sin leer se encuentran en el 1/200 de los datos totales. Una mayor proporción de decisiones hace que el sistema entre en <span class="wintitle"> Entrada rápida</span> con mayor facilidad, mientras que una relación inferior hace que sea menos probable que entre <span class="wintitle"> Entrada rápida</span> en el menú contextual. </p> <p> <p>Nota: Si el parámetro se establece en 0, el sistema no podrá entrar <span class="wintitle"> Entrada rápida</span> para el procesamiento inicial. Al establecer el parámetro en 1.1, el sistema puede introducir <span class="wintitle"> Entrada rápida</span> durante el procesamiento inicial, pero no para el procesamiento posterior. Adobe no recomienda el uso de valores entre 0 y 1.1. Para obtener más información sobre cómo configurar este parámetro, póngase en contacto con el Adobe. </p> </p> </td> 
   </tr> 
   <tr> 
   ...
   <tr> 
      ...
      <ul id="ul_569B90E9A85246F88906FA5444F8A93E"> 
       <li id="li_3EF182CEF4A44106B5267175EC62B9AB"> Clic con el botón derecho <span class="uicontrol"> Fuentes sin conexión</span>y haga clic en <span class="uicontrol"> Agregar nuevo</span> &gt; <span class="uicontrol"> Fuente</span>. </li> 
       <li id="li_E8FBA212F4784B1A830745A90BB3AF90"> En el parámetro para el nuevo origen, introduzca la máscara de la secuencia de registro. Para fuentes de registro de sensor con nombres de archivo con el formato YYYYMMDD-<i>SENSORID</i>.vsl, la máscara es <i>SENSORID.SENSORID</i> distingue entre mayúsculas y minúsculas. Para los orígenes de registro de archivos de registro, la máscara es la cadena extraída por la variable <span class="wintitle"> Patrón de máscara</span>. Consulte <a href="../../../home/c-dataset-const-proc/c-log-proc-config-file/c-log-sources.md#concept-3d4fb817c057447d90f166b1183b461e"> Archivos de registro</a>. </li> 
