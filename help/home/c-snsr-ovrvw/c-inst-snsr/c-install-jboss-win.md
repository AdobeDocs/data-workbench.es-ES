...
1. En el servidor JBoss, cree un directorio en el que instalar los archivos de programa de Sensor. Tenga en cuenta que la cola de discos reside en este directorio, por lo que asegúrese de que el dispositivo que elija tenga espacio suficiente para mantener una cola del tamaño que necesita.
...
Antes de modificar la secuencia de comandos de inicio, asegúrese de que la variable JAVA_HOME está definida en el entorno de Windows.

En el [!DNL run.bat] archivo (por ejemplo, C:\jboss-4.0.5.GA\bin\run.bat), agregue las siguientes líneas cerca del final del archivo justo antes de las líneas de &quot;eco&quot; que preceden al comando de inicio del servidor JBoss:
