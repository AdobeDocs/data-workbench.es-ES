...
<table id="table_5FF491A7A6C24E43A06A5C344BF05430"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Establezca este parámetro solo cuando trabaje con Adobe Consulting Services. </p> <p>Habilita el registro de depuración para el módulo Web y el transmisor. </p> <p>Este parámetro se utiliza cuando el <span class="wintitle"> sensor</span> no funciona correctamente. Una vez definido este parámetro, debe crear un archivo vacío en la ubicación de ruta especificada y otorgarle derechos de escritura a todos los usuarios. Por ejemplo (dentro de un shell unix en el servidor web): 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Deshabilita el campo especificado </p> <p>Los usuarios pueden eliminar los campos que no utilizan o que no desean almacenar. Si el campo toma valores de cadena, al desactivarlo se pasa una cadena en blanco. Si el campo toma valores numéricos, al desactivarlo se pasa el número cero (0). Puede desactivar los campos siguientes: 
     <ul id="ul_4537B345EFAE449A9946DA0B52EA5C43"> 
      ...
      <li id="li_6D647C845EB54B1B84C756DCDD7DD4CC">x-new-visitante </li> 
      ...
      <li id="li_A10438D2DEBB4ADFB574BF7094F9F0C4">cs-uri-consulta </li> 
      <li id="li_1D83BA59AC8543319D1966BB8ED1D931">s-dns </li> 
      <li id="li_34A23CE1944F4AEFBE7D74E8D6D5BEE6">cs(remitente del reenvío) </li> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Le permite desactivar el seguimiento de visitantes, que se puede utilizar para cumplir con las directivas de exclusión. </p> <p>Cuando está habilitado, <span class="wintitle"> Sensor</span> no registra un "paquete" para ningún visitante cuya cookie V1st está configurada en el valor de PrivacyID especificado. Dado que no se registra información para esos visitantes, no se envía información sobre esos visitantes al servidor <span class="keyword"> del área de trabajo de datos</span> para su procesamiento. </p> <p>Para habilitar esta función, debe completar los siguientes pasos: 
     <ol id="ol_5D658C5E4AB14F419029E0FFC52F1310"> 
      ...
      <li id="li_3E20F068C2F94512A92F284C80273B1C">Los propietarios de sitios Web deben escribir código para configurar las cookies de visitante (V1st) de manera que el valor del ID de la cookie coincida con el valor de PrivacyID definido como "<span class="filepath"> txlogd.conf</span>". </li> 
     </ol> </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Indica si se va a validar el servidor con el parámetro CertName </p> <p>El valor predeterminado es "activado". </p> <p>Ejemplo: <span class="filepath"> VerifyCertName en</span> </p> </td> 
  </tr> 
 </tbody> 
</table>
...
<table id="table_598C3CDA5AA440228AF88C3BE4A8F77C"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>Establezca este parámetro solo cuando trabaje con Adobe Consulting Services. </p> <p>Indica al <span class="wintitle"> sensor</span> de IIS cuál de los dos posibles "ganchos" de registro debe utilizarse para registrar un paquete </p> <p>Utilice este parámetro cuando el <span class="wintitle"> sensor</span> de IIS no esté registrando los paquetes correctamente. Este parámetro se establecería en "off" si el gancho de registro predeterminado no funcionaba correctamente. El valor predeterminado es "activado". </p> <p>Ejemplo: <span class="filepath"> IISCaptureBytesSent en</span> </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> IISUseAlternateHandler </td> 
   <td colname="col2"> <p>Establezca este parámetro solo cuando trabaje con Adobe Consulting Services. </p> <p>Indica al <span class="wintitle"> sensor</span> cuál de los dos posibles "ganchos" debe utilizarse para configurar la cookie v1st. </p> <p>Este parámetro se utiliza cuando el <span class="wintitle"> sensor</span> de IIS no está configurando correctamente la cookie v1st. Este parámetro se establecería en "yes" si el gancho predeterminado no configuraba correctamente la cookie v1st. El valor predeterminado es "no". </p> <p>Ejemplo: <span class="filepath"> IISUseAlternateHandler no</span> </p> </td> 
  </tr> 
  <tr> 
...
<table id="table_88696CCA93874BB683538BD614E07890"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>Establezca este parámetro solo cuando trabaje con Adobe Consulting Services. </p> <p>Indica al <span class="wintitle"> sensor</span> cuál de los dos posibles "ganchos" debe utilizarse para configurar la cookie v1st. </p> <p>Este parámetro se utiliza cuando el <span class="wintitle"> sensor</span> de Apache no está configurando correctamente la cookie v1st. Este parámetro se establecería en "yes" si el gancho predeterminado no configuraba correctamente la cookie v1st. El valor predeterminado es "no". </p> <p>Ejemplo: <span class="filepath"> ApacheUseAlternateHandler no</span> </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> ApacheUseAmbosHandlers </td> 
   <td colname="col2"> <p>Establezca este parámetro solo cuando trabaje con Adobe Consulting Services. </p> <p>Indica al <span class="wintitle"> sensor</span> que intente configurar la cookie v1st en ambos ganchos. </p> <p>Este parámetro se utiliza cuando el <span class="wintitle"> sensor</span> de Apache no está configurando correctamente la cookie v1st. El valor predeterminado es "sí". </p> <p>Si se establece en "sí" y la cookie v1st no se establece correctamente en el primer gancho, se utiliza el segundo gancho. Si se establece en "no", se establece el parámetro ApacheUseAlternateHandler para indicar qué gancho utilizar para configurar la cookie v1st. </p> <p>Ejemplo: <span class="filepath"> ApacheUseAmbosHandlers sí</span> </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>NewUserCacheControl </p> <p>CacheControl </p> </td> 
